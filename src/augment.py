...
class CenteredOddOneOutAugment:
    ...
    def _normalize(self, imgs: np.ndarray, meta: np.ndarray):
        if self.norm_stats is None:
            return imgs.astype(np.float32), meta.astype(np.float32)
    ...
    def _augment_one_image(self, img: np.ndarray) -> np.ndarray:
    ...
    def __call__(self, imgs: np.ndarray, label: int):
        ...
        for i in range(5):
            aug_img = self._augment_one_image(out[i])
            ...
            p = preprocess_single(
                ...
                hard_threshold=self.preprocess_cfg.hard_threshold,
            )

            proc_imgs.append(p["centered_raw"])
            proc_meta.append(p["meta"])

        out = np.stack(proc_imgs, axis=0).astype(np.float32)   # (5,H,W)
        meta = np.stack(proc_meta, axis=0).astype(np.float32)  # (5,M)
...
class OddOneOutDataset(Dataset):
    def __init__(self, x: np.ndarray, y: np.ndarray = None, augment=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment is not None and self.y is not None:
            imgs, meta, label = self.augment(imgs, label)
        else:
            raise RuntimeError("Use this dataset for raw training data with augment enabled.")

        sample = {
            "centered_raw": torch.from_numpy(imgs),   # (5,H,W)  <-- no unsqueeze
            "meta": torch.from_numpy(meta),           # (5,meta_dim)
        }
